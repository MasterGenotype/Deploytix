...
`Cargo.toml`'s `version` is the source of truth. A push to `main` whose version
has no matching tag yet cuts that release automatically; a push that does not
change the version stops at the resolve job and releases nothing. Ordinary
pushes are therefore silent, and re-running one cannot republish.

To cut the next release, in one commit on `main`:

```sh
# 1.4.0 -> 1.5.0 (minor is the default bump)
sed -i '0,/^version = /s/^version = .*/version = "1.5.0"/' Cargo.toml
cargo update -p deploytix          # syncs the version in Cargo.lock
git commit -am "release: v1.5.0"
git push origin main               # the workflow does the rest
```

Both files must move together: the workflow reads `Cargo.toml`, but a stale
`Cargo.lock` makes the build rewrite it and dirties the tree.

Three other triggers still work and always release, since each is an explicit
request: pushing a `v*` tag, pushing a `release/v*` branch (for environments
that cannot push tags), and `workflow_dispatch` with a tag input.

The tag must be `vMAJOR.MINOR.PATCH` and must equal `Cargo.toml`'s version. The
shape check rejects `git describe` output such as `v1.4.0-44-g1266050`, which
has been released by accident before. The version check exists because the
version is compiled into the binaries, so a mismatched tag would publish `vX`
binaries reporting `vY`; the run fails before building and names the version it
expected.
...
it carries no egui dependency. Release notes are generated by GitHub.
