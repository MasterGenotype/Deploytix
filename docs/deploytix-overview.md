...
- **`services.rs`** — `enable_service()` per init: runit creates `runsvdir/default/<svc>` symlink; openrc runs `rc-update add <svc> default` in chroot; s6 runs `s6 set enable <svc>` in chroot (upstream s6-frontend CLI; the staged changes are compiled into the boot database by a single `s6 set commit` in the finalize phase); dinit creates `boot.d/<svc>` symlink. Notably blacklists installation of `elogind-<init>` (conflicts with `seatd-<init>`) and skips enabling `elogind` (only the base PAM module is needed). `pacman -S --needed` installs the `<svc>-<init>` packages before enabling.
