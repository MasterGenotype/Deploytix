...
- **`src/install/basestrap.rs`** — builds the package list, ensures the custom `[deploytix]` repo is reachable (ISO embedded → pre-built `.pkg.tar.zst` cache → makepkg from source → local repo under /var + generated pacman.conf), ensures Arch `[extra]` repo is added when needed, runs `basestrap` with retry-on-network-error (max 3 attempts, 5 s delay).
...
- **`services.rs`** — `enable_service()` per init: runit creates `runsvdir/default/<svc>` symlink; openrc runs `rc-update add <svc> default` in chroot; s6 runs `s6 repository sync` + `s6 set enable <svc>` in chroot (upstream s6-frontend CLI; the staged changes are compiled and installed as the boot database by a single `s6 set commit` + `s6 live install --init` in the finalize phase); dinit creates `boot.d/<svc>` symlink. Notably blacklists installation of `elogind-<init>` (conflicts with `seatd-<init>`) and skips enabling `elogind` (only the base PAM module is needed). `pacman -S --needed` installs the `<svc>-<init>` packages before enabling.
...
- **`greetd.rs`** — writes `/etc/greetd/config.toml`, autologin tweaks, custom s6 service directory at `/etc/s6/adminsv/greetd` (no `greetd-s6` package exists in Artix repos).
...
| `/etc/pacman.conf` (host, runtime) | INI | May be augmented with `[deploytix]` and `[extra]` sections by `prepare_deploytix_repo` and `ensure_arch_repos`; modified copy written to `/var/cache/deploytix/pacman.conf` and passed to `basestrap -C` |
...
| `src/resources/gamescope_update/PKGBUILD` | Bash | Canonical gamescope rebuild for deployed systems (`deploytix-update-gamescope`); same meson options, https source. Also embedded in the installer to build `gamescope-git` on demand |
| `src/resources/custom_pkgbuilds/tkg-gui.PKGBUILD` | Bash | Embedded copy of `vendor/tkg-gui/pkg/PKGBUILD`; lets the installer build `tkg-gui-git` on demand without the submodule |
...
- `/var/cache/deploytix/pacman.conf` — generated pacman.conf (kept for the duration of the install).
- `/var/cache/deploytix/repo/` — local pacman repo built from located/built `.pkg.tar.zst` files. Under `/var` so it works on an immutable host (whose `/tmp` is read-only) and is visible from inside a transactional chroot.
...
| 8 | Custom repo prep | basestrap package list | `/var/cache/deploytix/pacman.conf` (+ optional `/var/cache/deploytix/repo/`) | `src/install/basestrap.rs:784 prepare_deploytix_repo`, `:891 ensure_arch_repos` |
