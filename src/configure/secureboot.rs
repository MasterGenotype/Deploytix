...
pub fn sign_boot_files(
    ...
    let boot_dir = format!("{}/boot", install_root);
    if let Ok(entries) = std::fs::read_dir(&boot_dir) {
        for entry in entries.flatten() {
            let name = entry.file_name().to_string_lossy().to_string();
            if name.starts_with("vmlinuz-") {
                let kernel_path = format!("/boot/{}", name);
                sign_efi_binary(cmd, config, &kernel_path, install_root)?;
            }
        }
    }

    // For sbctl, use sign-all to catch any missed files and add to database
    if config.system.secureboot_method == SecureBootMethod::Sbctl {
...
fn create_signing_hook(
    ...
    // Check if using standalone GRUB (sbctl + encryption)
    let use_standalone_grub =
        config.system.secureboot_method == SecureBootMethod::Sbctl && config.disk.encryption;

    let hook_content = match config.system.secureboot_method {
        SecureBootMethod::Sbctl => {
            if use_standalone_grub {
                // For standalone GRUB, we need to rebuild it when GRUB config changes
                // Create a helper script that rebuilds standalone GRUB and re-signs
                create_standalone_grub_rebuild_script(install_root)?;
                r#"[Trigger]
Operation = Install
Operation = Upgrade
Type = Path
...
[Action]
Description = Rebuilding standalone GRUB and signing for SecureBoot...
When = PostTransaction
Exec = /usr/local/bin/rebuild-grub-secureboot
Depends = sbctl
Depends = grub
"#
            } else {
...
/// Create script to rebuild standalone GRUB and re-sign for SecureBoot
fn create_standalone_grub_rebuild_script(install_root: &str) -> Result<()> {
    let script_dir = format!("{}/usr/local/bin", install_root);
    fs::create_dir_all(&script_dir)?;

    // GRUB modules list - same as in bootloader.rs
    let grub_modules = "all_video boot btrfs cat chain configfile echo \
    ...
    let script = format!(
        r#"#!/bin/bash
# Rebuild standalone GRUB and sign for SecureBoot
...
echo "Signing all SecureBoot files..."
sbctl sign-all

echo "SecureBoot rebuild complete"
"#,
        grub_modules
    );

    let script_path = format!("{}/rebuild-grub-secureboot", script_dir);
