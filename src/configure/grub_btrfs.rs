...
pub fn install_grub_btrfs(
    ...
    write_grub_regen_guard(cmd, config, install_root)?;
...
fn write_grub_btrfs_config(
    ...
    // On an immutable system grub-btrfsd must not run grub-mkconfig directly:
    // the live `/` is an overlay, where it silently produces a grub.cfg with no
    // snapshot menu (see `write_grub_regen_guard`).
    let mkconfig = if config.packages.immutable_root {
        GRUB_REGEN_GUARD
    } else if standalone {
        REINSTALL_GRUB
    } else {
...
/// Path of the regeneration guard installed on immutable systems.
pub const GRUB_REGEN_GUARD: &str = "/usr/local/bin/deploytix-grub-regen";

/// Rebuild-and-re-sign pipeline the installer writes on encrypted layouts.
const REINSTALL_GRUB: &str = "/usr/local/bin/reinstall-grub";

/// Write the guard that grub-btrfsd calls instead of `grub-mkconfig`.
///
/// A booted immutable root is an **overlayfs**, and both halves of GRUB
/// generation quietly give up on one:
///
/// - `grub-probe` aborts with *"failed to get canonical path of `overlay'"*, so
///   `10_linux` can emit a broken or empty grub.cfg;
/// - `41_snapshots-btrfs` tests `btrfs filesystem df /`, gets *"not a btrfs
///   filesystem"*, prints *"Root filesystem isn't btrfs"* and **exits 0** —
///   emitting no snapshot submenu at all.
///
/// grub-btrfsd is enabled on these systems and fires whenever the watched
/// snapshot directory changes, so without this guard it can overwrite the good
/// grub.cfg that `deploytix update`/`rollback` just wrote from a chroot — taking
/// the snapshot menu with it, and reporting success either way.
///
/// The guard refuses on an overlay root and otherwise runs the real command, so
/// it stays correct if the same system is later booted non-immutably.
fn write_grub_regen_guard(
    cmd: &CommandRunner,
    config: &DeploymentConfig,
    install_root: &str,
) -> Result<()> {
    ...
    let real = if uses_standalone_grub(config) {
        REINSTALL_GRUB.to_string()
    } else {
        format!("grub-mkconfig -o {}", crate::immutable::boot::GRUB_CFG)
    };

    info!("Writing {} (grub regeneration guard)", GRUB_REGEN_GUARD);
    if cmd.is_dry_run() {
        println!("  [dry-run] Would write {GRUB_REGEN_GUARD} wrapping: {real}");
        return Ok(());
    }

    let content = format!(
        r#"#!/bin/bash
# Generated by Deploytix — grub regeneration guard for immutable roots.
#
# A booted immutable `/` is an overlayfs. grub-probe cannot canonicalize one
# ("failed to get canonical path of `overlay'"), and grub-btrfs bails out of it
# ("Root filesystem isn't btrfs") while still exiting 0 — so a grub-mkconfig run
# here rewrites /boot/grub/grub.cfg without the snapshot menu, or empty, and
# reports success.
#
# The menu is regenerated by `deploytix update` and `deploytix rollback`, which
# chroot into the target snapshot set where / is a real btrfs root.

set -u

if [ "$(findmnt -no FSTYPE / 2>/dev/null)" = "overlay" ]; then
    echo "deploytix: / is an overlay (immutable root) — skipping grub regeneration." >&2
    echo "deploytix: run 'deploytix update' or 'deploytix rollback' to rebuild the snapshot menu." >&2
    exit 0
fi
...
"#
    ...
    let dir = format!("{install_root}/usr/local/bin");
    fs::create_dir_all(&dir)?;
    let path = format!("{install_root}{GRUB_REGEN_GUARD}");
    fs::write(&path, content)?;
...
mod tests {
    ...
    #[test]
    fn immutable_points_grub_btrfsd_at_the_guard_not_grub_mkconfig() {
        let root = temp_root("regen_guard");
        let cmd = CommandRunner::new(false);
        let mut cfg = base_config();
        cfg.packages.immutable_root = true;

        write_grub_regen_guard(&cmd, &cfg, &root).unwrap();
        write_grub_btrfs_config(&cmd, &cfg, &root).unwrap();

        // The daemon must never reach grub-mkconfig directly on these systems.
        let conf = read_config(&root);
        assert!(conf.contains(&format!("GRUB_BTRFS_MKCONFIG={GRUB_REGEN_GUARD}")));

        let guard = fs::read_to_string(format!("{root}{GRUB_REGEN_GUARD}")).unwrap();
        // Refuses on an overlay root — where grub-btrfs would exit 0 having
        // emitted nothing, quietly replacing a good grub.cfg.
        assert!(guard.contains("findmnt -no FSTYPE /"));
        assert!(guard.contains("overlay"));
        assert!(guard.contains("exit 0"));
        // ...and still works if the same system is booted non-immutably.
        assert!(guard.contains("exec grub-mkconfig -o /boot/grub/grub.cfg"));
        assert_valid_shell_file(&guard);

        // Executable, or the daemon cannot run it.
        use std::os::unix::fs::PermissionsExt;
        let mode = fs::metadata(format!("{root}{GRUB_REGEN_GUARD}"))
            .unwrap()
            .permissions()
            .mode();
    ...
    #[test]
    fn standalone_immutable_guard_wraps_the_resign_pipeline() {
        let root = temp_root("regen_guard_standalone");
        let cmd = CommandRunner::new(false);
        let mut cfg = base_config();
        cfg.packages.immutable_root = true;
        ...
        write_grub_regen_guard(&cmd, &cfg, &root).unwrap();
        let guard = fs::read_to_string(format!("{root}{GRUB_REGEN_GUARD}")).unwrap();
        // grub.cfg is embedded in a signed EFI binary here, so writing the
        // on-disk copy alone would never reach the boot menu.
        assert!(guard.contains("exec /usr/local/bin/reinstall-grub"));
        assert_valid_shell_file(&guard);
        let _ = fs::remove_dir_all(&root);
    }

    #[test]
    fn a_mutable_install_gets_no_guard() {
        let root = temp_root("regen_guard_mutable");
        let cmd = CommandRunner::new(false);
        let cfg = base_config(); // immutable_root defaults false
        write_grub_regen_guard(&cmd, &cfg, &root).unwrap();
        assert!(!std::path::Path::new(&format!("{root}{GRUB_REGEN_GUARD}")).exists());
        let _ = fs::remove_dir_all(&root);
    }

    fn assert_valid_shell_file(script: &str) {
