...
/// on encrypted and LVM-thin layouts: `grub-mkconfig`, then either
/// `grub-install` or a standalone rebuild, then SecureBoot re-signing.
///
/// Anything needing a *complete* boot-config rebuild runs this rather than a
/// bare `grub-mkconfig` — on a standalone install the config that boots is
/// embedded in the signed EFI binary, so a bare mkconfig writes a file nothing
/// reads and the menu never changes. Used by the Phase 5.45 regeneration,
/// `immutable::boot::activate_target`, and grub-btrfsd via
/// `GRUB_BTRFS_MKCONFIG`.
