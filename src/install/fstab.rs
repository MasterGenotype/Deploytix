...
use crate::configure::swap::{SWAP_FILE_PATH, swap_file_fstab_entry};
...
fn generate_fstab_with_subvolumes(
    ...
    info!("Generating /etc/fstab with {} btrfs subvolumes", subvolumes.len());
    ...
    if cmd.is_dry_run() {
        ...
        for sv in subvolumes {
            println!("    UUID=<ROOT_UUID> {} btrfs subvol={},{} 0 {}",
                sv.mount_point,
                sv.name,
                sv.mount_options,
    ...
    info!("Fstab written to {} with {} subvolume entries", fstab_path, subvolumes.len());
    Ok(())
}
...
pub fn generate_fstab_lvm_thin(
    ...
    if cmd.is_dry_run() {
        ...
        for vol in thin_volumes {
            println!("    /dev/{}/{} {} btrfs defaults,noatime,compress=zstd 0 {}",
                vg_name, vol.name, vol.mount_point,
    ...
    info!("Fstab written to {} with {} LVM thin volumes", fstab_path, thin_volumes.len());
    Ok(())
}
